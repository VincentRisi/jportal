...
//			Console.WriteLine("delimiterLength = " + delimiterLength);
		...
		// Vincent Risi: attempting to allow Span.Begin and Span.End to honour ignoreCase
		private bool CharCompare(bool ignoreCase, char ch1, char ch2)
		{
			if (ignoreCase) {
		...
		{
			return GetRegString(expr, index, document, false);
		}
		internal string GetRegString(char[] expr, int index, IDocument document, bool ignoreCase)
		{
			int j = 0;
			...
			for (int i = 0; i < expr.Length; ++i, ++j) {
				...
				switch (expr[i]) {
					...
					default:
						...
						if (CharCompare(ignoreCase, expr[i], document.GetCharAt(this.Offset + index + j)) == false) {
		...
		{
			return MatchExpr(expr, index, document, false);
		}
...
//									--j;
					...
					default:
						...
						if (index + j >= this.Length || CharCompare(ignoreCase, expr[i], document.GetCharAt(this.Offset + index + j)) == false) {
							return false;
						}
						break;
				}
