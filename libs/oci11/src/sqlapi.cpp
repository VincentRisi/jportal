...
static char *vbstrrtrim(char *s, int16 len)
{
  s[len - 1] = ' ';
...
static void vbstrrpad(char *s, int16 fldlen, char padchar)
{
  int len = strlen(s);
  while (len < fldlen)
    s[len++] = padchar;
...
static void compress(char *ZLOB)
{
...
static void uncompress(char *ZLOB)
{
  int32         *Lengths = (int32 *)ZLOB;
  unsigned char *Data    = (unsigned char *)ZLOB+8;
...
static pchar DynamicFieldValue(pSqlQuery aQuery, pchar token, pchar data)
{
  ...
  for (i=0; i < aQuery->NoFields; i++)
  {
    ...
    {
      data += Field->Offset;
      return vbstrrtrim(data, (int16)Field->Size);
    }
  }
  return "";
}
...
static void FudgeData(pSqlCursor SqlCursor)
{
  ...
  for (i=0; i<SqlQuery->NoFields; i++)
  {
    ...
    {
      isNull = *(int16*)(Data+Field->Offset+Field->Size);
      SqlCursor->Indicators[i*SqlCursor->RowsCount] = (int16) isNull ? -1 : 0;
    }
    if (isNull == 0)
    {
...
static void UnFudgeData(pSqlCursor SqlCursor)
{
  ...
  for (i=0; i<SqlQuery->NoFields; i++)
  {
    ...
    && ((Field->isBind|Field->isDefine) & fieldIsNullable)
    && ((Field->isBind|Field->isDefine) & fieldIsNull))
      *(int16*)(Data+Field->Offset+Field->Size) = isNull;
    if (isNull != 0)
    {
        switch(Field->SqlType)
        {
...
int16 APPLAPI SqlOneWayCipher(pchar Input, ushort InpLen,
