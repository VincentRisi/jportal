...
JSONCPP_STRING lintToString(LargestInt value) {
...
JSONCPP_STRING luintToString(LargestUInt value) {
  UIntToStringBuffer buffer;
  char* current = buffer + sizeof(buffer);
...
JSONCPP_STRING intToString(Int value) {
  return lintToString(LargestInt(value));
}

JSONCPP_STRING uintToString(UInt value) {
  return luintToString(LargestUInt(value));
}

#endif // # if defined(JSON_HAS_INT64)

//namespace {
JSONCPP_STRING doubleToStringSpecial(double value,
...
JSONCPP_STRING doubleToString(double value,
                             unsigned int precision,
                             PrecisionType precisionType) {
  return doubleToStringSpecial(value, false, precision, precisionType);
}

JSONCPP_STRING boolToString(bool value) { return value ? "true" : "false"; }
...
void FastWriter::writeValue(const Value& value) {
  ...
  case nullValue:
    ...
    break;
  case intValue:
    document_ += lintToString(value.asLargestInt());
    break;
  case uintValue:
    document_ += luintToString(value.asLargestUInt());
    break;
  case realValue:
    document_ += doubleToString(value.asDouble());
    break;
  case stringValue: {
  ...
  case booleanValue:
    document_ += boolToString(value.asBool());
    break;
  case arrayValue: {
    document_ += '[';
...
void StyledWriter::writeValue(const Value& value) {
  ...
  case nullValue:
    ...
    break;
  case intValue:
    pushValue(lintToString(value.asLargestInt()));
    break;
  case uintValue:
    pushValue(luintToString(value.asLargestUInt()));
    break;
  case realValue:
    pushValue(doubleToString(value.asDouble()));
    break;
  case stringValue: {
    // Is NULL possible for value.string_? No.
    char const* str;
  ...
  case booleanValue:
    pushValue(boolToString(value.asBool()));
    break;
  case arrayValue:
...
void BuiltStyledStreamWriter::writeValue(Value const& value) {
  ...
  case uintValue:
    ...
    break;
  case realValue:
    pushValue(doubleToStringSpecial(value.asDouble(), useSpecialFloats_, precision_,
                            precisionType_));
    break;
  case stringValue: {
