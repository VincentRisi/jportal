...
public class PopUbiServer3 extends Generator
{
  public static String description()
  {
    return "Generates Ubiquitous Server Code (AIX|SUN|NT)";
  }
  ...
  */
  ...
  private static boolean generatePyCCode(Module module, String output, PrintWriter outLog)
  {
    ...
    for (int i = 0; i < module.prototypes.size(); i++)
    {
      Prototype prototype = module.prototypes.elementAt(i);
    ...
    try
    {
      ...
      {
        outData.println("// This code was generated, do not modify it, modify it at source and regenerate it.");
        outData.println("#if defined(USE_" + module.name.toUpperCase() + "_STUBBED)");
        outData.println("#include \"" + module.name.toLowerCase() + ".h\"");
        outData.println();
        outData.println("void T" + module.name + "::SetupPythonInterp(Config *config)");
        outData.println("{");
        outData.println("  throw xIdlException(__FILE__, __LINE__, \"" + module.name + "\", 0, \"Module " + module.name + " SetupPythonInterp marked not used.\");");
        outData.println("}");
        outData.println();
        for (int i = 0; i < module.prototypes.size(); i++)
        {
          ...
          if (prototype.code.size() == 0 && prototype.isRpcCall())
          {
            PopUbiGen.generateUbiCImplCode(module, prototype, outData);
            outData.println("{");
            outData.println("  throw xIdlException(__FILE__, __LINE__, \"" + module.name + "\", 0, \"Module " + module.name + " Prototype " + prototype.name+ " marked not used.\");");
            outData.println("}");
            outData.println();
          }
        }
  ...
  public PopUbiServer3() {}
  private static PopUbiServer3 us = new PopUbiServer3(); 
...
//        return "PyObject* _item" + no + " = PyInt_FromLong((int64)" + ptr + field.name + ");";
      ...
      case Type.STRING:
        ...
        return "PyObject* _item" + no + " = PyBytes_FromString(" + field.name + ");"; // PyBytes_AS_STRING
      default:
  ...
  private static String outIntrinsic(String name, Type type, int no)
  {
    switch (type.typeof)
    {
      ...
      case Type.STRING:
        ...
        return name + " = PyBytes_AsString(_out" + no + ");"; // PyBytes_AS_STRING returns no errors
      default:
  ...
  private static void generateMethodCall(Module module, Prototype prototype, PrintWriter outData)
  {
    ...
    outData.println("  pythonModuleAuto _am(\"" + module.name.toUpperCase() + "_CODE\");"); 
  ...
  private static void generateCCode(Module module, String output, PrintWriter outLog)
  {
    try
    {
      ...
      try
      {
        ...
        outData.println();
        outData.println("#ifndef " + module.name.toUpperCase() + "_OVERRIDE_ADDLIST");
        ...
        outData.println("    THROW2(ADDLIST_ERROR, " + module.name.toUpperCase() + " \"Addlist Error List out of sync with Index\");");
        ...
        outData.println("      THROW2(ADDLIST_REALLOC_FAILED, " + module.name.toUpperCase() + " \"realloc ran out of memory\");");
  ...
  private static boolean generateCServer(Module module, String output, PrintWriter outLog)
  {
    ...
    try
    {
      ...
      try
      {
        ...
        outData.println("    IDL2_UBI_LOG((" + module.name + "->logFile->lprintf(eLogDebug, \"reqID=%d *** Unknown Message ***\", input->reqID)));");
