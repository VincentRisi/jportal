  ...
  static void generateVBHeader(Module module, String output, PrintWriter outLog)
  {
    try
    {
      ...
      try
      {
        ...
        outData.println("End Function");
        outData.println();
        outData.println("Public Sub "+module.name+"Logon(Service As String, Host As String, Optional Timeout As Long = 150000)");
        outData.println("  ErrorCode = "+module.name+"SnapLogon(fHandle, Service, Host, Timeout)");
        outData.println("  If ErrorCode <> 0 Then Err.Raise ErrorCode, \""+module.name+"\", \"Logon to \" & Service & \" \" & Host & \" failed\"");
