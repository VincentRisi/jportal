...
public class PopGenNTServer extends Generator
{
  public static String description()
  {
    return "Generates Server Code (NT)";
  }
  ...
  private static void generateCHeader(Module module, String output, PrintWriter outLog)
  {
    try
    {
      ...
      try
      {
        ...
        outData.println("#include \"snaprpcserver.h\"");
  ...
  private static boolean generateCServer(Module module, String output, PrintWriter outLog)
  {
    ...
    try
    {
      ...
      try
      {
        ...
        outData.println("{");
        outData.println("  LogFile->CustomString = \"\";");
        if (doConnect == true)
        {
        ...
        if (doConnect == true)
        {
          ...
          outData.println("        Result="+module.name.toUpperCase()+"_UNCAUGHT_DB_ERROR;");
          outData.println("      }");
        }
        outData.println("    }");
        if (hasShutDownCode)
  ...
  */
  private static void generateCServer(Module module, Prototype prototype, int no, PrintWriter outData)
  {
    ...
    if (Variants > 0)
    {
      ...
      outData.println("    {");
      //outData.println("      int i; //  DOF AIX Fix");
