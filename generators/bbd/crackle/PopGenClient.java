...
public class PopGenClient extends Generator
{
  ...
  public static void generateCClientMake(Module module, String output, PrintWriter outLog)
  {
    try
    {
      ...
      try
      {
        ...
        outData.println("# Preset BBDLIBS to latest TI Library eg. c:\\dev\\407\\.");
        outData.println("# Preset ORANO to 7 8 or 9 etc.");
        outData.println("# $(BCB) should be Preset.");
        outData.println("# If you want to use Microsoft C++ to compile build");
        outData.println("# a similar makefile. Or Build using an IDE.");
        ...
        outData.println();
        outData.println("CFLAGS = -c -Od -H- -Vx -Ve -X- -r- -a8 -b- -k -y -v -vi- -tWM -Tkh30000");
        outData.println();
        ...
        outData.println("\t$(CC) $(CFLAGS) -e"+module.name.toLowerCase()+".dll -Ls:$(BCB)\\lib @&&|");
        outData.println("$("+module.name.toUpperCase()+")");
        outData.println("$(LIBS)");
        outData.println("|");
        outData.println();
  ...
  public static void generateCClientHeader(Module module, String output, PrintWriter outLog)
  {
    try
    {
      ...
      try
      {
        ...
        outData.println("extern const int32 "+module.name+"Signature;");
  ...
  public static void generateCClient(Module module, String output, PrintWriter outLog)
  {
    try
    {
      ...
      try
      {
        ...
        outData.println("const int32 " + module.name + "Signature = " + module.signature + ";");
        outData.println();
        outData.println("const char *" + module.name + "Errors[] = ");
        ...
        for (int i = 0; i < module.tables.size(); i++)
        {
          ...
          String comma = "  ";
          outData.println("const char *"+module.name+table.name+"[] = ");
          outData.println("{");
        ...
        outData.println("  t"+module.name+"SnapCB(tString Service, tString Host, int32 Timeout)");
        w1 = ":";
        ...
        outData.println("  {");
        outData.println("    RPC = new tRPCClient(Host, Service, Timeout);");
        outData.println("  }");
        ...
        outData.println("};");
        outData.println();
        outData.println("const int CookieStart = 1719;");
        ...
        outData.println();
        outData.println("extern \"C\" e"+module.name+" APPLAPI "+module.name+"SnapLogon(int32* Handle, char* Service, char* Host, int32 Timeout)");
        outData.println("{");
        outData.println("  try");
        outData.println("  {");
        outData.println("    *Handle = "+module.name+"CBHandle.Create(new t"+module.name+"SnapCB(Service, Host, Timeout));");
  ...
  */
  public static void generateCClient(Module module, Prototype prototype, int no, PrintWriter outData)
  {
    ...
    outData.println("  {");
    if (alignForSun == true)
      outData.println("    snapCB->sendBuffLen = alignData(4);");
    else
    ...
    for (int i = 0; i < prototype.inputs.size(); i++)
    {
      ...
      ||  field.type.reference == Type.BYREFPTR)
      {
        if (input.hasSize() == false)
        {
          ...
          {
            if (alignForSun == true)
              outData.println("    snapCB->sendBuffLen += alignData(alignData(sizeof(int32))+strlen("+field.name+")+1);");
            else
          ...
          {
            if (alignForSun == true)
              outData.println("    snapCB->sendBuffLen += alignData(sizeof(*"+field.name+"));");
            else
        ...
        else
        {
          ...
          if (alignForSun == true)
            outData.println("    snapCB->sendBuffLen += alignData(alignData(sizeof(int32))+"+op.name+"*sizeof(*"+field.name+"));");
          else
    ...
    for (int i = 0; i < prototype.inputs.size(); i++)
    {
      ...
      ||  field.type.reference == Type.BYREFPTR)
      {
        ...
        else
        {
          ...
          if (alignForSun == true)
            outData.println("    "+w1+"ip += alignData(int32("+op.name+"*sizeof(*"+field.name+")));");
          else
    ...
    {
      hasRX = true;
      outData.println("    if (snapCB->RPC->RxSize())");
      outData.println("    {");
      ...
      for (int i = 0; i < prototype.outputs.size(); i++)
      {
        ...
        ||  field.type.reference == Type.BYREFPTR)
        {
          ...
          else
          {
            ...
            if (alignForSun == true)
              outData.println("      "+w1+"ip += alignData(snapCB->recvSize);");
            else
